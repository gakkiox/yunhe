  ...
  <div class="min-h-screen bg-background text-foreground transition-colors duration-300 flex flex-col">
    ...
    <nav class="nav-header backdrop-blur-md bg-background/80 border-b border-border">
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        <div class="flex items-center gap-3 cursor-pointer" @click="switchToSearch" title="强制刷新">
          ...
          <div>
            <h1 class="text-xl font-bold">云河搜索</h1>
          </div>
        </div>

        <!-- 导航菜单 -->
        <nav class="flex items-center gap-2">
          <button @click="switchToSearch" class="nav-button" :class="{ 'active': currentPage === 'search' }" title="搜索">
            <span class="nav-icon">
          ...
          <button v-if="isAuthenticated" @click="handleLogout" class="nav-button logout-button"
    ...
    <main class="container mx-auto px-4 py-8 flex-1">
      ...
      <div v-if="currentPage === 'search'" class="search-page">
        <!-- 收藏提示 -->
        <h2 class="text-sm  text-center" style="margin-bottom: 12px;">海量网盘资源一站即享，做好用的网盘搜索引擎
          按Ctrl+D收藏我们！朋友圈分享本站可以免费领取阿里云盘超级会员月卡。</h2>
        ...
        <div v-if="hasSearched || loading" class="mb-6">
          <SearchStats :total="searchResults.total || 0" :mergedResults="searchResults.mergedResults || {}"
            :loading="loading" :searchTime="searchTime" :isUpdating="isUpdating" :updateCount="updateCount"
        ...
        <!-- 搜索结果 -->
        <div v-else>
          <ResultTabs :mergedResults="searchResults.mergedResults || {}" :loading="loading" :hasSearched="hasSearched"
      ...
      <div v-else-if="currentPage === 'accounts'" class="accounts-page">
        <AccountCenter :backend-health="backendHealth" @navigate="handleAccountNavigate" />
      </div>
    ...
    <footer class="border-t border-border bg-background/50 backdrop-blur-sm mt-auto">
      <div class="container mx-auto px-4 py-4">
        ...
        <div class="flex items-center justify-center gap-4 text-sm text-muted-foreground">
          ...
          <a href="https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzg4MzcxNDAzOA==#wechat_redirect"
            target="_blank" rel="noopener noreferrer" class="hover:text-foreground transition-colors">公众号：慢笔生花</a>
        </div>
        <img class="hidden" src="/logo.png" alt="云河搜索">
      </div>
    </footer>
  </div>
</template>
