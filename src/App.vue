    ...
    <footer class="border-t border-border bg-background/50 backdrop-blur-sm mt-auto">
      <div class="container mx-auto px-4 py-4">
        <div class="text-sm text-muted-foreground flex items-center justify-center ">
          <div>声明：本站内容由网络爬虫自动抓取。本站不储存、复制、传播任何文件，不做任何盈利，仅作个人公益学习，请勿非法&商业传播，如有侵权，请反馈告知删除。</div>
        </div>
        <img class="hidden" src="/logo.png" alt="云河搜索">
        <div class="flex items-center justify-center gap-4 text-sm text-muted-foreground">
          <span>© {{ new Date().getFullYear() }}-{{ new Date().getFullYear() + 10 }}</span>
          <a href="https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzg4MzcxNDAzOA==#wechat_redirect" target="_blank" rel="noopener noreferrer" class="hover:text-foreground transition-colors">公众号：慢笔生花</a>
        </div>
      </div>
    </footer>
  </div>
